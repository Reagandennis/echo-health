...
// Companion read for updateSessionTracksAction. The browser Appwrite client has
// no session attached (the cookie is httpOnly), so an ACL'd getDocument from the
// client returns 404 — this reads the session's published-track fields via the
// admin client after verifying the caller is the patient/therapist on the session.
export async function getSessionTracksAction(
  ...
  const sess = await databases.getDocument(
    ...
    appwriteConfig.collections.sessions,
    sessionId
  ) as unknown as { patientId: string; therapistId: string; patientTracks?: string; therapistTracks?: string };
