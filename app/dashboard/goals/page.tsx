    ...
    <div className={`bg-white rounded-2xl border p-5 ${isComplete ? "border-emerald-200" : "border-brand/10"}`}>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${goal.assignedBy === "therapist" ? "bg-brand/10 text-brand" : "bg-cream text-stone-500"}`}>
                {goal.assignedBy === "therapist" ? "Therapist assigned" : "Self-set"}
              </span>
              {milestones.length > 0 && (
                <span className="text-xs text-stone-400">{done}/{milestones.length} steps</span>
              )}
            </div>
          </div>
        </div>
        {milestones.length > 0 && (
          <button onClick={() => setExpanded((v) => !v)} className="text-stone-500 hover:text-brand transition-colors mt-1" aria-label="Toggle milestones">
      ...
      {milestones.length > 0 && (
        <div className="mt-3">
          ...
          <p className="text-[10px] text-stone-400 mt-1">{pct}% complete</p>
        </div>
      )}
      {/* Milestones */}
          ...
          {milestones.map((m, i) => (
            <button key={m.title} onClick={() => void toggleMilestone(i)}
              ...
              {m.completed
                ...
                : <Circle size={16} className="text-stone-400 group-hover:text-stone-500 transition-colors shrink-0" />}
              <span className={`text-sm ${m.completed ? "text-stone-500 line-through" : "text-stone-600"}`}>{m.title}</span>
            </button>
          ))}
        </div>
      )}
    ...
    <dialog open className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-transparent m-0 max-w-none max-h-none w-full h-full">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10">
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor="goal-title" className="block text-xs font-semibold text-stone-500 uppercase tracking-wide mb-1.5">Goal</label>
            <input id="goal-title" value={title} onChange={(e) => setTitle(e.target.value)}
              placeholder="e.g. Reduce anxiety in social situations"
              className="w-full rounded-xl border border-brand/15 px-3 py-2.5 text-sm text-brand placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-brand/30" />
          </div>
          <div>
            <label htmlFor="goal-desc" className="block text-xs font-semibold text-stone-500 uppercase tracking-wide mb-1.5">
              Description <span className="normal-case font-normal text-stone-400">(optional)</span>
            </label>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-xs font-semibold text-stone-500 uppercase tracking-wide">Milestones</span>
              <button type="button" onClick={addMilestone} className="text-xs text-brand hover:text-stone-700 transition-colors flex items-center gap-1">
              ...
              {milestones.map((m) => (
                <div key={m.id} className="flex gap-2">
                  <input value={m.text} onChange={(e) => updateMilestone(m.id, e.target.value)}
                    ...
                    className="flex-1 rounded-xl border border-brand/15 px-3 py-2 text-sm text-brand placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-brand/30" />
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-7">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-0.5">{active.length} active · {completed.length} completed</p>
        </div>
        ...
        <div className="space-y-3">
          {(tab === "active" ? active : completed).length === 0 ? (
            <div className="flex flex-col items-center py-16 text-center">
              ...
              <p className="text-sm text-stone-500">No {tab} goals</p>
              {tab === "active" && (
                <button onClick={() => setShowAdd(true)}
